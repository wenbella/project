...
from utlis import save_checkpoint, AverageMeter, visualize_image, \
...
def main(evaluate, epochs, start_epoch, batch_size, workers, print_freq, lr, weight_decay):
    ...
    # Create model
    model = ResidualEncoder(out_channels=2)
    ...
    #         checkpoint = torch.load(resume) if use_gpu else torch.load(
    ...
    if not evaluate:
        ...
        train_imagefolder = GrayscaleImageFolder(train_directory,
                                                 train_transforms)
    ...
    #     save_images = True
    #     epoch = 0
    #     initial_losses = validate(val_loader, model, criterion, save_images,
    #                               epoch)
    #
    ...
    #     return
        ...
        validate(val_loader, model, criterion, False, 0, print_freq)  # validate before training
        for epoch in range(start_epoch, epochs):
            # Train for one epoch, then validate
            train(train_loader, model, criterion, optimizer, epoch, print_freq)
            save_images = True  # (epoch % 3 == 0)
            losses = validate(val_loader, model, criterion, save_images, epoch, print_freq)
...
def validate(val_loader, model, criterion, save_images, epoch, print_freq):
    ...
    for i, (input_gray, input_ab, target) in enumerate(val_loader):
        ...
        input_gray_variable = Variable(input_gray,
                                       volatile=True).cuda() if use_gpu else Variable(
            input_gray, volatile=True)
        input_ab_variable = Variable(input_ab,
                                     volatile=True).cuda() if use_gpu else Variable(
            input_ab, volatile=True)
        ...
        if save_images:
            for j in range(len(output_ab)):
                ...
                visualize_image(input_gray[j], ab_input=output_ab[j].data,
                                show_image=False, save_path=save_path,
                                save_name=save_name)
...
if __name__ == '__main__':
    ...
    lr = 0.1
    weight_decay = 1e-10
    main(evaluate, epochs, start_epoch, batch_size, workers, print_freq, lr, weight_decay)